...
3. **Typed extension points, not arbitrary DOM.** A plugin declares _what_ it contributes (nav items, pages, cards) through a closed, versioned vocabulary. The portal stays in control of where plugin UI can appear.
...
```json
{
  ...
  "extensions": [
    ...
    },
    {
      ...
      "requirements": {
        "permissions": [{ "group": "compute.miloapis.com", "resource": "instances", "verb": "get" }]
      }
    }
  ]
}
...
- **`comingSoon` + optional `comingSoonMode` / `roadmapUrl` / `description`** — when `comingSoon` is true, the host shows a Coming Soon badge until the project has an Active `ServiceEntitlement` whose `status.serviceName` matches `serviceRef` (canonical reverse-DNS, e.g. `compute.datumapis.com`; defaults to the plugin slug). While Coming Soon, `comingSoonMode` chooses the destination: `holding` (default) opens the host `/coming-soon/:id` page (`description` as body, `roadmapUrl` as optional “Learn more” CTA); `plugin` navigates to `path` (plugin landing / request-access UI); `external` opens `roadmapUrl` in a new tab (`roadmapUrl` required). Once entitled, the host uses `path` and drops the badge — no portal nav PR. Host-owned planned items (no plugin) always use the holding page. Live nav and `comingSoonMode: plugin` require a non-empty `path`; holding/external soft-launch may omit it.
- **`children` (optional)** nests items under this one, at any depth. A non-empty `children` array makes the item a collapsible parent with no link of its own; each child is `{ title, path?, order?, children? }`, where `path` is mount-relative like the parent's and is required unless the child has its own children. Children are text-only (no icons) and sort by `order` among siblings. Keep declaring the parent's `path`: older hosts ignore `children` and render the parent as a plain link to it. While Coming Soon, the badge stays on the parent and children render only in `comingSoonMode: plugin`; `holding` and `external` render the parent as a plain Coming Soon link.
...
| ----------------------------------- | ------ | ----------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `portal.nav/project`                | **v1** | Item in the project sidebar under a host category or a per-plugin group | `id`, `title`, `icon` (lucide name), `path`, `section?`, `order?`, `comingSoon?`, `comingSoonMode?`, `description?`, `roadmapUrl?`, `serviceRef?`, `children?` |
...
- **Platform-backed registry via `datumctl auth update-kubeconfig`.** The portal's `kubeconfig` registry source supports Kubernetes **exec credential plugins**, and `datumctl auth update-kubeconfig` writes exactly that: a kubeconfig whose credentials are fetched (and auto-refreshed) by datumctl. Pointing `PLUGIN_REGISTRY_KUBECONFIG` at such a kubeconfig makes the _same code path_ that watches the local kwok registry watch a real platform control plane — no tokens copied anywhere.
...
6. **Pre-release staging path** — once a plugin passes the local Tier 1 loop, what's the process for registering the service in the staging catalog so the team can validate with real entitlements before production?
7. **Interim catalog mirror** — `app/features/quotas/service-catalog.ts` should collapse onto `PortalPlugin`/`Service` display metadata once this ships; that migration should be sequenced with this work.
