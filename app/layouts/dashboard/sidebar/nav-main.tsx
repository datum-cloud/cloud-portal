...
export const NavMain = forwardRef<
  HTMLUListElement,
  ComponentProps<'ul'> & { items: NavItem[] }
>(({ className, items, ...props }, ref) => {
  ...
  const activeNavItem = useCallback(
    (val: string) => {
      ...
      const cleanVal = normalizedVal.endsWith('/')
        ? normalizedVal.slice(0, -1)
        : normalizedVal
  ...
  const renderNavItem = (item: NavItem, level = 0) => {
    ...
    const isOpen =
      openItems[item.href as string] || Boolean(pathnameExistInDropdowns.length)
