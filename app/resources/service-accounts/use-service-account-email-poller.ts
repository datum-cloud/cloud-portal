...
export function useServiceAccountEmailPoller(
  projectId: string,
  ...
  useEffect(() => {
    ...
    if (!projectId || !serviceAccountName) {
      return;
    }
    ...
    async function poll() {
      ...
      try {
        const account = await service.get(projectId, serviceAccountName);

        if (cancelledRef.current) return;

        if (account.identityEmail) {
          queryClient.setQueryData(
            serviceAccountKeys.detail(projectId, serviceAccountName),
            account
          );
  ...
  }, [projectId, serviceAccountName, initialEmail, queryClient]);
