...
export function createHttpProxyService() {
  return {
    ...
    async createTrafficProtectionPolicy(
      ...
      const baseURL = getProjectScopedBase(projectId);
      const body = toTrafficProtectionPolicyPayload(httpProxyName, mode);
    ...
    async updateTrafficProtectionPolicyMode(
      ...
      try {
        await patchNetworkingDatumapisComV1AlphaNamespacedTrafficProtectionPolicy({
          baseURL,
          path: { namespace: 'default', name },
          // Merge patch updating just spec.mode
          body: { spec: { mode } },
        ...
        if (status === 404) {
          ...
          await this.createTrafficProtectionPolicy(projectId, name, mode);
        } else {
          throw error;
        }
      }
    ...
    async update(
      ...
      try {
        ...
        // If WAF mode was changed, update the TrafficProtectionPolicy
        if (input.trafficProtectionMode) {
          try {
