...
export const HttpProxyUpstreamResponse = ({
  ...
  const query = useMemo(() => {
    return `sum(rate(envoy_vhost_vcluster_upstream_rq{resourcemanager_datumapis_com_project_name="${projectId}",gateway_name="${proxyId}",gateway_namespace="default",label_topology_kubernetes_io_region!=""}[1m])) by (label_topology_kubernetes_io_region,envoy_response_code)`;
  }, [projectId, proxyId]);
