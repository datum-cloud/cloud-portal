...
export const OVERVIEW_RANGE_OPTIONS = [
  { value: '5m', label: 'Last 5 minutes', ms: 5 * 60 * 1000, step: '15s' },
...
export function useOverviewRange(value: OverviewRangeValue): OverviewRange {
  ...
  return useMemo(() => {
    const option =
      OVERVIEW_RANGE_OPTIONS.find((item) => item.value === value) ??
      OVERVIEW_RANGE_OPTIONS.find((item) => item.value === DEFAULT_OVERVIEW_RANGE) ??
