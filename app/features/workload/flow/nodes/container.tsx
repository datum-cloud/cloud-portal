...
export const ContainerNode = ({ data }: { data: IContainerNode }) => {
  const baseClass = 'relative rounded-lg border border-purple-200 bg-purple-50 px-4 py-2 shadow-md';
...
export const ContainerGroupNode = ({ data }: { data: IGroupNode }) => {
  const baseClass = 'relative rounded-lg border border-purple-500 bg-purple-50 px-4 py-2 shadow-md';
  ...
  return (
    <div className={`${baseClass} ${sizeClass}`}>
      <Handle type="target" position={Position.Top} id="target" className="h-4 w-4 bg-purple-500" />
      <Handle
        type="source"
        position={Position.Bottom}
        id="source"
      ...
      <div className="flex items-center justify-between">
        <div className={`font-bold ${fontClass} text-center text-purple-700`}>{data.label}</div>
        <Button
          type="quaternary"
          theme="borderless"
          size="icon"
