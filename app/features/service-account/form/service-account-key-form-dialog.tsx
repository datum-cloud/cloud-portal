...
export interface ServiceAccountKeyFormDialogRef {
  show: () => void;
  hide: () => void;
}

interface ServiceAccountKeyFormDialogProps {
  projectId: string;
  serviceAccountId: string;
  serviceAccountEmail: string;
  onKeyCreated?: (response: CreateServiceAccountKeyResponse) => void;
}

type KeyType = 'datum-managed' | 'user-managed';

const KEY_FORM_DEFAULTS: ServiceAccountKeyCreateSchema = {
  name: '',
  type: 'datum-managed',
  publicKey: '',
  expiresAt: '',
...
export const ServiceAccountKeyFormDialog = forwardRef<
  ServiceAccountKeyFormDialogRef,
  ServiceAccountKeyFormDialogProps
>(({ projectId, serviceAccountId, serviceAccountEmail, onKeyCreated }, ref) => {
  const [open, setOpen] = useState(false);
  const [keyType, setKeyType] = useState<KeyType>('datum-managed');

  const createMutation = useCreateServiceAccountKey(
    projectId,
    serviceAccountId,
    serviceAccountEmail,
    {
      onSuccess: (response) => {
        toast.success('Key created', {
          description: 'Service account key has been created successfully.',
        });
        setOpen(false);
  ...
  const handleSubmit = useCallback(
    async (data: ServiceAccountKeyCreateSchema) => {
...
ServiceAccountKeyFormDialog.displayName = 'ServiceAccountKeyFormDialog';
