...
export interface ServiceAccountFormDialogRef {
  show: (initialValues?: ServiceAccount) => void;
  hide: () => void;
}
...
const CREATE_DEFAULTS: ServiceAccountCreateSchema = {
  name: '',
  displayName: '',
};
...
export const ServiceAccountFormDialog = forwardRef<
  ServiceAccountFormDialogRef,
  ServiceAccountFormDialogProps
>(({ projectId, onCreated }, ref) => {
  const [open, setOpen] = useState(false);
  const [editName, setEditName] = useState('');
  const [defaultValues, setDefaultValues] = useState<ServiceAccountCreateSchema>(CREATE_DEFAULTS);

  const isEdit = !!editName;

  const createMutation = useCreateServiceAccount(projectId, {
    onSuccess: (newAccount) => {
      toast.success('Service account', {
        description: 'Service account has been created successfully.',
      });
      setOpen(false);
  ...
  const updateMutation = useUpdateServiceAccount(projectId, editName, {
    onSuccess: () => {
      toast.success('Service account', {
        description: 'Service account has been updated successfully.',
      });
      setOpen(false);
    },
    onError: (error) => {
  ...
  const handleSubmit = useCallback(
    async (data: ServiceAccountCreateSchema | ServiceAccountUpdateSchema) => {
      if (isEdit) {
        updateMutation.mutate({ displayName: data.displayName });
      } else {
        createMutation.mutate(data as ServiceAccountCreateSchema);
      }
    },
  ...
  return (
    <Form.Dialog
      ...
      title={isEdit ? 'Edit Service Account' : 'New Service Account'}
      description={
        isEdit
          ? 'Edit the service account with the new values below.'
          : 'Create a new service account to get started.'
      }
      schema={isEdit ? serviceAccountUpdateSchema : serviceAccountCreateSchema}
...
ServiceAccountFormDialog.displayName = 'ServiceAccountFormDialog';
