 ...
 * entered. Business name wins when present (legal entity / company identity);
 * otherwise contact name. An explicit `displayNameOverride` always wins over
 * both (see `useSetupOnboardingBilling`).
 */
...
type AddressField = 'line1' | 'line2' | 'city' | 'region' | 'postalCode';

/** Merge-patch payload for Organization.spec.contactInfo (RFC 7386 null deletes). */
export type OrganizationContactInfoPatchPayload = {
  email: string;
  name: string;
...
const patchAddressField = (
  values: OrgContactInfoValues,
  previous: OrganizationContactInfoPayload | null | undefined,
  field: AddressField
): string | null | undefined => {
  const next = values[field]?.trim() ?? '';
  if (next) return next;
  if (previous?.address?.[field]?.trim()) return null;
  return undefined;
};

/**
 * Maps contact form values to a merge-patch payload. Emits explicit `null` for
 * fields the user cleared so the server removes them (RFC 7386).
 */
export const toOrganizationContactInfoPatch = (
  values: OrgContactInfoValues,
  previous: OrganizationContactInfoPayload | null | undefined
): OrganizationContactInfoPatchPayload => {
  const patch: OrganizationContactInfoPatchPayload = {
    email: values.email.trim(),
    name: values.name.trim(),
  };
  ...
  } else if (previous?.businessName?.trim()) {
    patch.businessName = null;
  }

  const country = values.country?.trim() ?? '';
  const previousCountry = previous?.address?.country?.trim();
  ...
  const address: NonNullable<OrganizationContactInfoPatchPayload['address']> = { country };

  for (const field of ['line1', 'line2', 'city', 'region', 'postalCode'] as const) {
    const value = patchAddressField(values, previous, field);
    if (value !== undefined) {
      address[field] = value;
    }
  }
