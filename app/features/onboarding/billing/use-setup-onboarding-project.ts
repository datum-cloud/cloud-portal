...
import { awaitProjectReady } from '@/resources/projects/project.watch';
...
const inFlightSetupByOrgId = new Map<string, Promise<OnboardingProjectSetup>>();

async function runSetupOnboardingProject(
  ...
  const existingProjects = await createProjectService().listAll(input.orgId);
  ...
  if (!project) {
    ...
    project = await awaitProjectReady(input.orgId, created.name, created);
  } else {
    project = await awaitProjectReady(input.orgId, project.name, project);
  }
...
export async function setupOnboardingProject(
  ...
  const inFlight = inFlightSetupByOrgId.get(input.orgId);
  if (inFlight) {
    return inFlight;
  }

  const work = runSetupOnboardingProject(input).finally(() => {
    inFlightSetupByOrgId.delete(input.orgId);
  });

  inFlightSetupByOrgId.set(input.orgId, work);
  return work;
}
...
export async function completeLegacyOrgSetup(
  ...
  // Persist contact info onto the org itself — the setup gate reads it from
  // `org.spec.contactInfo`, so without this the org keeps looking incomplete
  // and the user gets bounced back into onboarding. Display name is left as-is.
  await createOrganizationService().updateContactInfo(input.orgId, {
    contactInfo: toOrganizationContactInfo(input.contactInfo),
  });
...
export function useSetupOnboardingProject(
  ...
  return useMutation({
    mutationKey: onboardingProjectSetupMutationKey,
