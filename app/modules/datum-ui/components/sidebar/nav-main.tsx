...
>(({ className, items, overrideState, itemClassName, disableTooltip, ...props }, ref) => {
    ...
    (item: NavItem) => {
      ...
      const isTabChildMatch =
        item.tabChildLinks?.some((childPath) => {
          ...
          return (
            cleanCurrentPath === cleanChildPath || cleanCurrentPath.startsWith(`${cleanChildPath}/`)
          );
        }) ?? false;
                      ...
                      <DropdownMenu key={`collapsed-item-${subItem.title}-${level}`}>
                        <DropdownMenuTrigger asChild className="w-full">
                          <DropdownMenuItem
                            className={cn(
                              'cursor-default',
                              (isSubItemActive || hasActiveSubChild) && 'bg-primary/10 text-primary'
                            )}>
