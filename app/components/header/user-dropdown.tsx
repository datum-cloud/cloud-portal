...
export const UserDropdown = () => {
  ...
  const fullName = useMemo(() => `${user?.given_name} ${user?.family_name}`, [user]);
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent
        ...
        <DropdownMenuGroup>
          ...
          <DropdownMenuItem
            className="cursor-pointer"
            onClick={() => navigate(routes.account.tokens.root)}>
            <KeyIcon />
            API Keys
          </DropdownMenuItem>
        </DropdownMenuGroup>
        ...
        <DropdownMenuItem
          ...
          onClick={() => {
            fetcher.submit(null, { method: 'POST', action: routes.auth.logOut });
          }}>
          <Button
            type="submit"
            variant="link"
