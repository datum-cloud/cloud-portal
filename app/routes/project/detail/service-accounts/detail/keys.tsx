...
export default function ServiceAccountKeysPage() {
  ...
  const pollerResult = useServiceAccountEmailPoller(
    projectId ?? '',
    serviceAccountId ?? '',
    serviceAccount?.identityEmail
  );

  const resolvedEmail = pollerResult.email ?? '';

  const { data: keys = [] } = useServiceAccountKeys(
    projectId ?? '',
    serviceAccountId ?? '',
    resolvedEmail
  );

  const revokeMutation = useRevokeServiceAccountKey(projectId ?? '', serviceAccountId ?? '', {
    onSuccess: () => {
  ...
  const rowActions: ActionItem<ServiceAccountKey>[] = useMemo(
    () => [
      {
        label: 'Revoke',
        variant: 'destructive',
  ...
  const columns: ColumnDef<ServiceAccountKey>[] = useMemo(
    () => [
      ...
      createActionsColumn<ServiceAccountKey>(rowActions),
    ],
    [rowActions]
  );
