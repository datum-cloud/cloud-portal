...
**Module Federation remote** that the cloud portal loads at runtime. It
exercises every v1 extension point AND Milo-mediated data fetching, so it
doubles as living documentation for plugin authors and as the end-to-end test
...
- `portal.card/project-home` — a home-page card showing a **live DNS zone count**
...
Every API call a plugin issues — including calls to a service's own aggregated
apiserver — goes through the portal's existing authenticated proxy at
`/api/proxy/apis/resourcemanager.miloapis.com/v1alpha1/projects/<id>/control-plane/…`.
There is no plugin-declared backend proxy: a plugin's data must live behind a
Milo (or Milo-aggregated) control plane. The "Platform data" page demonstrates
this by listing DNS zones at
`.../control-plane/apis/dns.networking.miloapis.com/v1alpha1/namespaces/default/dnszones`.

Data fetching uses `@tanstack/react-query` (a host-shared singleton), so plugin
queries live in the host's cache next to built-in pages. Plugins must **not**
...
`vite`) and never touches the root `package.json`. Install and run it in
isolation:

```bash
bun install
bun run dev      # Vite dev server (:7777, HMR) — standalone/direct
bun run preview  # built dist/ served (:7777) — proxy-safe
bun run build    # static dist/ (remoteEntry.js + plugin-manifest.json + chunks)
```

From the repo root: `task plugin:dev`, `task plugin:preview`, `task plugin:build`.
...
5. For data, call `/api/proxy/…` — the portal's existing Milo control-plane
   proxy, including for a service's own aggregated apiserver. Add
...
is served (`pluginCommand` / ready URL). No changes to `harness.ts` or
`plugin.spec.ts` needed.
...
preview port (7777), and the shared `.devenv` kwok cluster — so don't run it at
the same time as the cloud-portal repo's own `test:e2e:plugins`. Override with
`PORTAL_REPO_DIR`, `E2E_PORTAL_PORT`, `E2E_PLUGIN_PORT`, or `E2E_PROJECT_ID` as
needed.
